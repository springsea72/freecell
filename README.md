...
- 按 `Ctrl+Z` 撤销上一步操作。
- 点击“求解播放”会在后台求解当前牌局，找到路径后自动逐步播放。
- 播放过程中可以点击“暂停/继续”或“停止播放”。
- 播放过程中会禁用手动拖拽和右键归堆，避免破坏当前求解路径。
- 可以通过“播放间隔(ms)”滑块调整自动播放速度。
- 求解成功且可复现时，可以点击“保存 trace”导出路径，也可以点击“加载 trace 播放”复现已保存路径。
- 点击“重开一局”开始新牌局。
...
- 支持后台求解当前牌局并播放求解路径。
- 显示 GUI 求解参数和求解结果统计，包括 explored/generated/frontier/path length。
- 支持保存可复现求解 trace，以及加载并播放已验证 trace。
- 通过历史快照实现撤销。
...
- GUI 自动播放目前只支持 solver 路径播放，尚未接入 policy player。
